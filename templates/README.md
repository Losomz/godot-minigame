...
# 使用项目自己的裁切参数文件（任意路径）
python ci/package.py --template 4.5.2 --variant glx --profile ./my_project_trim.py --revision 1
```
...
- `templates/configs/*.py` 每个文件 = 一份裁切参数（`disable_*` / `module_*` 布尔开关）。
- 复用方式：
  - 复制：`cp templates/configs/wechat_2d.py templates/configs/my_trim.py`，修改开关；
