...
- Reads `GameGlobal.__GODOT_DISABLE_WXGLX === true` before creating a context. Set it in a module imported before `godot-loader.js` to force standard `webgl2` for an affected device rollout; it is a startup-only switch.
