...
def load_manifest(bundle: str) -> tuple[Path, dict]:
    manifest_path = ADAPTER_ROOT / "patches" / "manifest.json"
    with manifest_path.open("r", encoding="utf-8") as fh:
        manifest = json.load(fh)
    if manifest.get("bundle") != bundle:
        raise RuntimeError(
            f"Requested bundle {bundle!r} differs from manifest bundle {manifest.get('bundle')!r}"
        )
...
def main() -> int:
    ...
    source_root = ADAPTER_ROOT / "sources"
    ...
    print(f"  2. Post-process with: node {ADAPTER_ROOT / 'scripts' / 'godot_process.js'}")
    print(f"  3. Compress wasm with: cmd /c {ADAPTER_ROOT / 'scripts' / 'compress_wasm.bat'}")
    return 0
