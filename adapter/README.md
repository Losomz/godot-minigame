# Godot 4.6 WeChat Minigame Adapter

This directory is an open-source, self-contained adapter kit. It must work from:

- an official Godot checkout at the bundled base commit
- this repository's `adapter/` directory
...
Read `adapter/references/compatibility-matrix.md` before editing if the target is not exactly on that base.
...
2. **Near-base refresh**
   ...
   - Expect manual conflict resolution and replay only after checking `adapter/references/recent-commit-deltas.md`.

## Hard Rules

- Treat `adapter/patches/` and `adapter/sources/` as the authoritative public package.
...
- Do not describe this adapter as “supports all Godot 4.x” or “works on any 4.6 build” unless you actually verified that target.
...
```powershell
python adapter\scripts\apply_godot_patchset.py <target-repo>
```
...
```powershell
python adapter\scripts\install_min_runtime.py <dest-dir>
```
...
node adapter\scripts\godot_process.js
cmd /c adapter\scripts\compress_wasm.bat
```
...
```bash
sh adapter/scripts/compress_wasm.sh
```

### 5. Validate

- Read `adapter/references/validation-checklist.md`.
- For WXGLX-capable builds, also read and run the dual-path checks in `adapter/references/wxglx-adaptation.md`.
...
- `adapter/references/validation-checklist.md`
  - Build, packaging, and smoke-test gates.

## Output Standard

When using this adapter on a real repo, always leave behind:
