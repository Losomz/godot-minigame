...
Use the checkout recorded by `adapter/thirdparty/godot` and the maintained adaptation sources under `adapter/`. Do not treat this Skill directory as an adapter source package, and do not clone another Godot checkout into an Agent temporary directory.

## Source Locations

- Godot source and build tree: `adapter/thirdparty/godot/`
...
- Keep production code under `adapter/`, never under this Skill directory.
...
- Do not initialize `adapter/thirdparty/godot` unless engine development or template compilation requires it.
